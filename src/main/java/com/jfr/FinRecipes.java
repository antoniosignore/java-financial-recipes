...
import com.jfr.matrix.Matrix;
import com.jfr.termStructure.TermStructure;
...
public class FinRecipes extends FinMath implements FinConstants {
    ...
    //  call on a zero coupon bond.
    {
        ...
        int no_call_steps = (int) Double.doubleToLongBits(no_steps * option_maturity / bond_maturity);
        ...
        for (int n = 0; n < no_sims; n++) {
            switch (payoff) {
                ...
                case PAYOFF_EUROPEAN_PUT:
                    ...
                    break;
                default:
                    System.out.println("payoff not applicable");
                    break;
            }
        }
        System.out.println("----------------------------");
        System.out.println("time" + time);
        System.out.println("sum_payoffs" + sum_payoffs);
        System.out.println("no_sims" + no_sims);
    ...
    public static void simulate_price_sequence(double S, // current value of underlying
        ...
        for (int i = 0; i < no_steps; ++i) {
            S_t = S_t * Math.exp(R + SD * random.nextGaussian());
            prices[i] = S_t;
        }
    }
    ...
    public static double simulate_terminal_price(double S,
        ...
        return S * Math.exp(R + SD * random.nextGaussian());
    }
    ...
    public static double optionPriceCallEuropeanSimulated(double S,
        ...
        for (int n = 1; n <= no_sims; n++) {
            double S_T = S * Math.exp(R + SD * random.nextGaussian());
