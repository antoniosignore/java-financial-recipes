package com.jfr.matrix;

import com.jfr.FinMath;
...
/*
    public void doView(HttpServletRequest request, HttpServletResponse response, Writer out) throws Throwable
    {
        ...
        for (int i = 0; i < rows; i++)
        {
            Row row = table.generateRow();
            for (int j = 0; j < cols; j++)
            {
                Column col = row.generateColumn();
                col.setContent(new HTMLString(format.format(A[i][j])));
            }
        }
...
/*
    public void doView(String[] rows, String[] columns, HttpServletRequest request, HttpServletResponse response, Writer out) throws Throwable
    {
        DecimalFormat format = new DecimalFormat();
        format.setMinimumIntegerDigits(1);
        format.setMaximumFractionDigits(3);
        ...
        for (int j = 0; j < cols; j++)
        {
            Column col = row.generateColumn();
            col.setWidth(null);
            col.setContent(columns[j]);
        }

        for (int i = 0; i < this.rows; i++)
        {
            row = table.generateRow();
            Column col = row.generateColumn();
            col.setWidth(null);
            col.setContent(rows[i]);
            for (int j = 0; j < cols; j++)
            {
                col = row.generateColumn();
                col.setWidth(null);
                col.setContent(new HTMLString(format.format(A[i][j])));
            }
        }
    ...
    public void doView(String[] columns, HttpServletRequest request, HttpServletResponse response, Writer out) throws Throwable
    {
        DecimalFormat format = new DecimalFormat();
        format.setMinimumIntegerDigits(1);
        format.setMaximumFractionDigits(3);
...
*/
    ...
    /*   public Matrix inverse()
        {
            if (rows != cols) throw new IndexOutOfBoundsException();
            double ad[][] = new double[rows][cols];
            ...
            Matrix matrix = join(this, new Matrix(ad));
            for (int l = 0; l < rows; l++)
            {
                if (matrix.elementAt(l + 1, l + 1) == 0.0D)
                {
                    int i;
                    for (i = l + 1; i < rows && matrix.elementAt(i + 1, l + 1) == 0.0D; i++);
                    if (i >= rows) throw new ArithmeticException();
                    matrix.rowExchange(l + 1, i + 1);
                }
                matrix.rowMultiply(1.0D / matrix.elementAt(l + 1, l + 1), l + 1);
                for (int i1 = 0; i1 < l; i1++)
                    matrix.rowAddMultiple(-matrix.elementAt(i1 + 1, l + 1), l + 1, i1 + 1);
                for (int j1 = l + 1; j1 < rows; j1++)
                    matrix.rowAddMultiple(-matrix.elementAt(j1 + 1, l + 1), l + 1, j1 + 1);
            }
            return matrix.getSubMatrix(1, cols + 1, rows, cols);
        }
    */
